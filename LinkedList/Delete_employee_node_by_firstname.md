# C Program to Delete an Employee Node by First Name in a Linked List
...
struct Employee* deleteByFirstName(struct Employee *head, char firstName[]) {
    struct Employee *temp = head, *prev = NULL;
    while (temp != NULL) {
        char nameCopy[50];
        strcpy(nameCopy, temp->name);
        char *token = strtok(nameCopy, " ");
        if (token != NULL && strcmp(token, firstName) == 0) {
            if (prev == NULL) {
                head = temp->next;
            } else {
                prev->next = temp->next;
...
int main() {
    ...
    char firstName[50];
    printf("\nEnter the first name of the employee to delete: ");
    scanf("%s", firstName);

    head = deleteByFirstName(head, firstName);
...
Enter the first name of the employee to delete: Balu
