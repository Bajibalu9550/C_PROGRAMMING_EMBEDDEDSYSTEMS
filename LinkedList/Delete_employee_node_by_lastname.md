# C Program to Delete an Employee Node by Last Name in a Linked List
...
struct Employee* deleteByLastName(struct Employee *head, char lastName[]) {
    ...
    while (temp != NULL) {
        ...
        if (lname != NULL && strcmp(lname, lastName) == 0) {
            if (prev == NULL) {
                head = temp->next;
            } else {
                prev->next = temp->next;
...
Enter the last name of the employee to delete: Mehta
