# C Program to Sort Employee Nodes by Last Name in a Linked List
...
struct Employee* sortByLastName(struct Employee *head) {
    if (head == NULL) return NULL;
    struct Employee *i, *j;
    for (i = head; i != NULL; i = i->next) {
        for (j = i->next; j != NULL; j = j->next) {
            if (strcmp(getLastName(i->name), getLastName(j->name)) > 0) {
                int tempId = i->id;
...
int main() {
    ...
    printf("\nEmployee List After Sorting by Last Name:\n");
    displayEmployees(head);
